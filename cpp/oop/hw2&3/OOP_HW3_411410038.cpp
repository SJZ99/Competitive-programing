...
class event;
class link; // new
class GraphNode; // data record
class Graph; // sink will construct a graph after AGG
...
class GraphNode {
    unsigned int parent, dataSize;

    set<unsigned int> neighbor;
    set<unsigned int> child;

    unsigned int bestParent;
    set<unsigned int> bestChild;

    unsigned int total;

public:
    GraphNode(unsigned int parent = BROADCAST_ID, int dataSize = 0) {
        this->parent = parent;
        this->child = child;
        this->dataSize = dataSize;
    }

    void addNeighbor(unsigned int nei) {
        neighbor.insert(nei);
    }

    void setData(unsigned int dataSize) {
        this->dataSize = dataSize;
        this->total = dataSize;
    }

    void increaseData(unsigned int delta) {
        this->total += delta;
    }

    void decreaseData(unsigned int delta) {
        this->total -= delta;
    }

    void addChild(unsigned int chi) {
        child.insert(chi);
    }

    void removeChild(unsigned int chi) {
        child.erase(chi);
    }

    unsigned int countPacket(unsigned int packetCapacity, int offset = 0) {
        unsigned int tmp = total + offset;

        if(tmp % packetCapacity == 0) {
            return max(tmp / packetCapacity, 1U);
        } else {
            return max(tmp / packetCapacity + 1, 1U);
        }
    }
    ...
    void replaceBest() {
        bestParent = parent;
        bestChild = move(child);
        child = set<unsigned int>();
    }

    friend class Graph;
    friend class IoT_sink;
};

class Graph {
    vector<GraphNode> v;
    vector<int> bfsOrder;
    unordered_set<unsigned int> path;

    unsigned int dataPerPacket;

    void buildFlow();
    
    int tryRemoveFrom(unsigned int child, unsigned int from);

    int tryAddTo(unsigned int child, unsigned int to, unsigned int originParent);

    void reParent(unsigned int child, unsigned int from, unsigned int to);

    int calcFlow();

    void constructBfsTree(bool reverseOrder);
...
public:
    Graph(unsigned int size = 0) {
        v.resize(size);
        bfsOrder.reserve(size);
    }

    void setDataPerPacket(unsigned int dpp) {
        this->dataPerPacket = dpp;
    }

    void setNodeDataSize(unsigned int id, unsigned int dataSize) {
        if(id >= v.size()) { // check wether size is enough
            v.resize(id + 1);
            v[id].setData(dataSize);
        } else if(v[id].dataSize == 0){ // check wether first time adding
            v[id].setData(dataSize);
        }
    }
...
class IoT_ctrl_payload : public payload {
    ...
    public:
        ...
        bool isUpdateParent() { return setParent; }
        void updatedParent(unsigned int parent, int cnt) { setParent = true; newParent = parent; updateCnt = cnt; }
        void recvUpdateParent() { setParent = false; }
        unsigned int getPreParent() { return newParent; }
        int getUpdateCnt() { return updateCnt; }
...
class DIS_ctrl_payload : public payload {
    ...
    public:
        ...
        void setChild(const set<unsigned int>& chi) {
            child = set<unsigned int>(chi.begin(), chi.end());
        }

        const set<unsigned int>& getChildren() {
            return child;
        }
...
class node {
    ...
    public:
        ...
        virtual void send_handler(packet *P);
...
class IoT_device: public node {
        // one hop neighbors
        set<unsigned int> one_hop_neighbors;
        
        // ----- about ctrl -----

        unsigned int parent = 0;
        set<unsigned int> child;
        map<unsigned int, bool> recieved;
        map<unsigned int, int> childUpdateCnt;
        bool myDataReady = false;
        int minHop = BROADCAST_ID;
        int updateCnt = 0;

        // ----------------------

        // ----- about AGG -----
        map<unsigned int, unsigned int> reverse;

        // ----------------------
        string neighborToMessage();

    protected:
        IoT_device() {} // it should not be used
        IoT_device(IoT_device&) {} // it should not be used
        IoT_device(unsigned int _id): node(_id) {} // this constructor cannot be directly called by users
        ...
        void addRecv(unsigned int node) {
        ...
        void clearRecv() { 
            for(auto p = recieved.begin(); p != recieved.end(); p++) {
                p->second = false;
            }
        }
        ...
        bool canSend() {
            bool ok = true;
            for(unsigned int chi : child) {
                ok &= recieved[chi];
            }
            return ok && myDataReady;
        }

        void setMyDataStatus(bool stat) {
            myDataReady = stat;
        }

        int getAndIncreaseUpdateCnt() {
            return updateCnt++;
        }

        int getChildUpdate(unsigned int chi) {
            if(childUpdateCnt.find(chi) == childUpdateCnt.end()) {
                return -1;
            }

            return childUpdateCnt[chi];
        }

        void setChildUpdate(unsigned int chi, int cnt) {
            childUpdateCnt[chi] = max(childUpdateCnt[chi], cnt);
        }

        void addNeighbor(unsigned int nei) {
            one_hop_neighbors.insert(nei);
        }

        void addReverse(unsigned int through, unsigned int to) {
            reverse[to] = through;
        }

        unsigned int getReversePath(unsigned int to) {
            return reverse[to];
        }
    ...
    public:
        ...
        bool isMyChild(unsigned int chi) {
            return child.find(chi) != child.end();
        }

        int getChildNumber() { return child.size(); }

        bool isMyDataReady() { return myDataReady; }
...
class IoT_sink: public node {
        ...
        void analysisMessage(unsigned int src, string message);
    ...
    protected:
        ...
        unsigned int getReversePath(unsigned int to) {
            if(one_hop_neighbors.find(to) != one_hop_neighbors.end()) {
                return to;
            }
            return reverse[to];
        }
...
string IoT_device::neighborToMessage() {
    ...
    // put one hop neighbor, and separated by space
    for(unsigned int nei: one_hop_neighbors) {
        ss << nei << " ";
    }
...
void IoT_device::send_handler(packet *p){
    packet *_p = packet::packet_generator::replicate(p);

    if(_p->type() == "IoT_data_packet") { // data packet

        if(canSend()) {
            // if can send, clear pkg that has recieved
            // and set current pkg to my own data pkg
            clearRecv();

            // after sending to parent, reset status of myself data
            setMyDataStatus(false);
        } else {
            // skip this packet (from child)
            return;
        }
    }
...
void IoT_device::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet") { // the device receives a packet from the sink
        ...
        header* h3 = p3->getHeader();

        addNeighbor(h3->getPreID());

        // if can't update my parent, ignore packet
        bool updateParent = false;

        // if previous node modify its parent, then try to update child
        if(l3->isUpdateParent() && l3->getUpdateCnt() > getChildUpdate(h3->getPreID())) {

            unsigned int pre = h3->getPreID();
            unsigned int newParent = l3->getPreParent();

            if(newParent == getNodeID()) { // add child
                addChild(pre);
            } else if(newParent != getNodeID() && isMyChild(pre)) { // remove child
                removeChild(pre);
            }

            l3->recvUpdateParent();
            setChildUpdate(h3->getPreID(), l3->getUpdateCnt());
        }

        // check wether need to update my parent (compared by hop cnt, then node id)
        if(l3->getCounter() < minHop || (l3->getCounter() == minHop && parent > h3->getPreID())) {
            updateParent = true;

            // update parent
            parent = h3->getPreID();
            minHop = l3->getCounter();
            ...
            // modify package to notify neighbor my parent has changed
            l3->updatedParent(parent, getAndIncreaseUpdateCnt());
        }
        ...
        header* h = p3->getHeader();

        // my own data packet is generated
        if(h->getSrcID() == getNodeID()) {
            this->setMyDataStatus(true);
        } 

        // if this packet's src is not me, mean it's not my data packet
        // and if the dst is me, mean it is from my child
        // update I have which child's data 
        if(h->getNexID() == getNodeID()) {
            this->addRecv(h->getPreID());
        }

        // update packet header
        h->setPreID( getNodeID() );
        h->setNexID( this->parent );
        h->setDstID( 0 );

        // try send to parent only when all data from children has recieve
        // the packet will be send
        send_handler(p3);
    }
        ...
        // send packet to other, and record reverse path

        // my own packet:
        if(h->getSrcID() == getNodeID()) {
            l3->setMsg( neighborToMessage() );

            h->setPreID( getNodeID() );
            h->setNexID( this->parent );
            h->setDstID( 0 );
        } else { // from other devices
            addReverse( h->getPreID(), h->getSrcID() );
            h->setPreID( getNodeID() );
            h->setNexID( this->parent );
            h->setDstID( 0 );
        }
        ...
        // this packet is not belong me
        if( h->getDstID() != getNodeID() ) {
            h->setPreID( getNodeID() );
            h->setNexID( getReversePath(h->getDstID()) );
            send_handler( p3 );
        } else if( h->getDstID() == getNodeID() ) { // this is my packet!
            // update parent
            this->parent = l3->getParent();

            // update child
            this->child = l3->getChildren();
        }
    }
};

void IoT_sink::analysisMessage(unsigned int src, string message) {
    stringstream ss(message);
    unsigned int size, nei;
    ss >> size;
    g.setNodeDataSize(src, size);
...
void IoT_sink::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet") { // the device receives a packet from the sink
        ...
        addNeighbor(p3->getHeader()->getPreID());
        ...
        // if this is first ctrl packet, then send
        if(!hi) {
            hi = true;
            send_handler(p3);
        }
        ...
        // analysis packet from IoT_device
        analysisMessage(p3->getHeader()->getSrcID(), l3->getMsg());
    }
    else if (p->type() == "DIS_ctrl_packet") {
        DIS_ctrl_packet *p3 = nullptr;
        p3 = dynamic_cast<DIS_ctrl_packet*> (p);
        ...
        // start calculate new parent
        if(h->getSrcID() == getNodeID()) {

            g.setDataPerPacket(dataPerPacket);
            vector<GraphNode>& v = g.v;

            // add sink's neighbor to graph
            g.setNodeDataSize( getNodeID(), dataSize );
            for(auto nei : one_hop_neighbors) {
                g.addEdge( getNodeID(), nei );
            }
            ...
            for(long unsigned int i = 1; i < v.size(); ++i) {
                l3->setParent(v[i].bestParent);
                l3->setChild(v[i].bestChild);

                h->setSrcID( 0 );
                h->setPreID( getNodeID() );
                h->setNexID( getReversePath(i) );
                h->setDstID( i );
...
void Graph::calculateParent() {
    int flow = INT_MAX, tmp;

    for(int i = 0; i < 2; ++i) {

        constructBfsTree(i % 2);
        optimize();
...
// construct bfs tree and store bfs order to bfs order array
void Graph::constructBfsTree(bool reverseOrder) {
    ...
    while (!q.empty()) {
        ...
        std::set<unsigned int>& neighbor = v[curr].neighbor;
...
int Graph::calcFlow() {
    ...
    for(int i = 1; i < v.size(); ++i) {
        flow += v[i].countPacket(dataPerPacket);
    }

    return flow;
}

// calculate data packet size to sink
void Graph::buildFlow() {

    for(int i = bfsOrder.size() - 1; i >= 0; --i) {
        unsigned int id = bfsOrder[i];

        for(unsigned int chi : v[id].child) {
            v[id].increaseData(v[chi].total);
        }
    }
}

int Graph::tryRemoveFrom(unsigned int child, unsigned int from) {

    int delta = 0;

    int childData = v[child].total;
    unsigned int curr = from, pa = v[curr].parent;

    while(curr != 0) {
        int originPacket = v[curr].countPacket(dataPerPacket);
        int newPacket = v[curr].countPacket(dataPerPacket, -childData);
        delta += newPacket - originPacket;
...
int Graph::tryAddTo(unsigned int child, unsigned int to, unsigned int originParent) {
    int delta = 0;

    int childData = v[child].total;

    unsigned int curr = to, pa = v[curr].parent;
...
void Graph::reParent(unsigned int child, unsigned int from, unsigned int to) {

    int childData = v[child].total;

    // remove from original parent
    unsigned int curr = from, pa = v[curr].parent;
    // cout << "make " << child << " from " << from << " to " << to << "\n";

    while(curr != 0) {
        v[curr].decreaseData(childData);
    ...
    // add to new parent
    curr = to, pa = v[curr].parent;

    while(curr != 0) {
        v[curr].increaseData(childData);

        curr = pa;
        pa = v[curr].parent;
    }

    v[from].removeChild(child);
    v[to].addChild(child);
    v[child].parent = to;
}
...
void Graph::optimize() {
    ...
    for(int i = v.size() - 1; i >= 0; --i) {
        ...
        // this case may can be optimized
        if((v[id].total % dataPerPacket) <= (dataPerPacket >> 1)) {
            unsigned int newParent = BROADCAST_ID, childNeedChange = BROADCAST_ID;
            int minn = INT_MAX;
            
            for(unsigned int chi: v[id].child) {
                int afterRemoved = tryRemoveFrom(chi, id);

                // may can be better
                if(afterRemoved < 0) {
                    // try to find new parent
                    for(unsigned int nei : v[chi].neighbor) {
                        // skip original parent
                        if(nei == id || v[chi].child.find(nei) != v[chi].child.end()) continue;

                        // calculate #packet after adoption
                        int afterAdd = tryAddTo(chi, nei, id);
                        // cout << chi << " " << afterRemoved << " " << afterAdd << "\n";
                        if(afterAdd + afterRemoved < minn) {
                            minn = afterAdd + afterRemoved;
                            childNeedChange = chi;
                            newParent = nei;
                        }
                    }
            ...
            if(newParent != BROADCAST_ID && minn < 0) {
                reParent(childNeedChange, id, newParent);
            }
        }
    }
...
int main()
{
    ...
    int nodeN, linkN, packetSize;
    int simTime, bfsTime, aggTime, disTime, transTime;
    cin >> nodeN >> linkN >> packetSize;
    cin >> simTime >> bfsTime >> aggTime;
    cin >> disTime >> transTime;
    
    // read the input and generate devices
    node::node_generator::generate("IoT_sink", 0);

    IoT_sink* sink = dynamic_cast<IoT_sink*>(node::id_to_node(0));
    sink->setDataPerPacket(packetSize);
    
    for (unsigned int id = 1; id < nodeN; id++) {
    ...
    for(int i = 0; i < nodeN; ++i) {
        ...
        int size;
        cin >> id >> size;
        node::id_to_node(id)->setDataSize(size);
    }
    
    // set devices' neighbors
    for(int i = 0, linkId, end1, end2; i < linkN; ++i) {
        cin >> linkId >> end1 >> end2;
        node::id_to_node(end1)->add_phy_neighbor(end2);
        node::id_to_node(end2)->add_phy_neighbor(end1);
    }
    
    // node 0 broadcasts a msg with counter 0
    IoT_ctrl_packet_event(0, bfsTime);
    // 1st parameter: the source; the destination that want to broadcast a msg with counter 0 (i.e., match ID)
    // 2nd parameter: time (optional)
    // 3rd parameter: msg for debug information (optional)
    
    // AGG
    for(int id = 1; id < nodeN; ++id) {
        AGG_ctrl_packet_event(id, 0, aggTime);
    ...
    // DIS
    DIS_ctrl_packet_event(0, disTime);

    // data trans
    for(int id = 1; id < nodeN; ++id) {
        IoT_data_packet_event(id, 0, transTime);
        // 1st parameter: the source node
        // 2nd parameter: the destination node (sink)
        // 3rd parameter: time (optional)
    ...
    cout << sink->getNodeID() << " " << sink->getParentID() << "\n";
    
    for(int id = 1; id < nodeN; ++id) {
        IoT_device* node = dynamic_cast<IoT_device*>(node::id_to_node(id));
        cout << node->getNodeID() << " " << node->getParentID() << "\n";
    }
