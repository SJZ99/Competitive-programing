...
class IoT_device: public node {
    ...
    protected:
        ...
        void clearRecv() { recieved.clear(); }

        bool canSend() {
            return child.size() == recieved.size() && myDataReady;
        }
...
int main()
{
    ...
    int nodeN, linkN;
    int simTime, startTime, transTime;
    cin >> nodeN >> linkN;
    cin >> simTime >> startTime >> transTime;
    ...
    IoT_ctrl_packet_event(0, startTime);
    // 1st parameter: the source; the destination that want to broadcast a msg with counter 0 (i.e., match ID)
    // 2nd parameter: time (optional)
